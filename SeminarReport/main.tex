...
\begin{abstract}
  ...
  This report illustrates features of \texttt{LiquidHaskell} through a small formalization and demonstrate its application with several examples. 
...
Generally, there are two approaches: static and dynamic verification.
Dynamic verification tests program correctness during execution by providing inputs and checking the outputs. 
...
This approach includes techniques such as model checking and deductive verification.
In deductive verification, the user supplies the program's specifications, and the verifier checks if the program satisfies them \cite{wiki_software_2024}.

Two main trends in deductive verifiers are Satisfiability Modulo Theory (SMT)-based and Typed-Theory (TT)-based approaches. 
...
In this report, after a short overview on refinement types and SMT solvers in section \ref{sec:background}, 
...
Refinement types extend conventional type systems by attaching logical predicates to types \cite{vazou_refinement_2014}. For example:  
...
If we use static verifier, this only allows applications
of the function that satisfy the preconditions, ensuring that the function is safe to call
and will not result in a divide-by-zero runtime error \cite{vazou_refinement_2014}. 

\subsection{\textbf{LiquidHaskell} and SMT}
