...
Program verification is the process of proving or disproving that a program conforms to its specifications.
Generally, there are two approaches: \textit{static} and \textit{dynamic} verification.
Dynamic verification, commonly known as the Test phase, tests program correctness during execution by providing inputs and checking the outputs. 
This approach is not exhaustive and may not cover all possible inputs.
In contrast, static verification uses mathematical logic to establish correctness without executing the program.
This approach includes techniques such as \textit{model checking} and \textit{deductive verification}.
In deductive verification, the user supplies the program's specifications, and the verifier checks if the program satisfies them \cite{SoftwareVerification2024}.
...
whereas SMT-based verifiers often use SMT solvers to check the satisfiability of universally-quanitified axioms—axioms 
...
Refinement types combine the best of both worlds by fusing types with SMT based validity checking \cite{vazou_refinement_2018}.
...
integer as its first argument and a positive integer as its second argument. 
The refinement type system then generates \emph{verification conditions} 
(VCs)—logical formulas whose validity ensures that the type specification is 
satisfied \cite{vazou_refinement_2014}. 
When the function is called, the checker verifies that these VCs hold.

\begin{haskell}  
bad :: Nat -> Nat -> Int 
bad x y = x `div` y
\end{haskell} 
For instance, the checker, based on the type specification, rejects the \texttt{bad} function.
Before generating any VCs, however, the refinement type system translates the annotation into a so-called
\textit{subtyping} query as following \cite{vazou_refinement_2014}:
...
Refinement type systems are designed to exclude any arbitrary functions and only includes formulas
