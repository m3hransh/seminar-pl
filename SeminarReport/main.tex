...
\begin{abstract}
  ...
  Refinement types are types that extends the expressiveness of Haskell types systems by providing predicates that can verify
  nvariants of the program. This report explains briefly how SMT solvers are leveraged by \texttt{LiquidHaskell} and 
...
verify the program in section \ref{sec:lh}. Then in section \ref{sec:example} we will provide some examples of how to use 
...
These are refinements of the basic \texttt{Int} type with logical predicates stating that the 
value \texttt{v} is strictly positive and non-negative, respectively. 
...
and will not throw a divide-by-zero runtime error \cite{vazou_refinement_2014}.  
...
such that three clauses conjunction :
...
By adding these dependencies, \texttt{LiquidHaskell} can now check your program at compile time or via a code linter in your IDE of choice.  
Take note that there are some options, such as \texttt{reflection} and \texttt{ple}, that can be used to enable reflection and PLE in \texttt{LiquidHaskell}.  
