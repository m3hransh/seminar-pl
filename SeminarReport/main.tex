...
\begin{haskell}
  ...
  x ? _ = x
\end{haskell}
\texttt{Proof} is a type alias for the unit type, and \texttt{(***)} is a function that takes a value of type \texttt{a} and a value of type \texttt{QED} and returns a value of type \texttt{()}. 
The \texttt{(***)} function is used to mark the end of a proof. 
The \texttt{(?)} function is the \textit{because} combinator that even though it ignores its second argument, 
it is used to add a fact in the proof . 
The \texttt{===} function is used to prove that two values are equal.
